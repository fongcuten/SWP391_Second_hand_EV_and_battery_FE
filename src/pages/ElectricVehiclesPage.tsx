...
const ElectricVehiclesPage: React.FC = () => {
          ...
          (postsResponse.content || []).map(async (post) => {
            if (post.provinceCode && post.districtCode && post.wardCode) {
              try {
                ...
                console.error(
                  `Error converting location for post ${post.listingId}:`,
                  locationError
                );
              }
            }
  ...
  useEffect(() => {
    if (filters.selectedProvinceCode) {
      ...
      locationService
        .getDistricts(filters.selectedProvinceCode)
        .then(setDistricts)
        .catch(() => setDistricts([]))
  ...
  useEffect(() => {
    if (filters.selectedDistrictCode) {
      setLoadingLocations(true);
      locationService
        .getWards(filters.selectedDistrictCode)
        .then(setWards)
        .catch(() => setWards([]))
        .finally(() => setLoadingLocations(false));
    } else {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-xl text-green-100 max-w-2xl mx-auto">
              Khám phá {filteredPosts.length} xe điện chất lượng cao với giá tốt
              nhất
            </p>
          </motion.div>
        </div>
          ...
          >
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
              <FilterSelect
                ...
                value={filters.selectedProvinceCode || ""}
                onChange={(e) =>
                  handleFilterChange(
                    "selectedProvinceCode",
                ...
                defaultLabel="Tất cả tỉnh"
              />
              <FilterSelect
                icon={MapPin}
                label="Quận/Huyện"
                value={filters.selectedDistrictCode || ""}
                onChange={(e) =>
                  handleFilterChange(
                    "selectedDistrictCode",
                    e.target.value ? Number(e.target.value) : null
                ...
                disabled={!filters.selectedProvinceCode || loadingLocations}
              ...
              <FilterSelect
                icon={MapPin}
                label="Phường/Xã"
                value={filters.selectedWardCode || ""}
                onChange={(e) =>
                  handleFilterChange(
                    "selectedWardCode",
                    e.target.value ? Number(e.target.value) : null
                ...
                disabled={!filters.selectedDistrictCode || loadingLocations}
              ...
              <FilterSelect
                ...
                disabled={!filters.selectedBrandId}
                options={models.map((m) => ({
                  value: m.modelId,
                  label: m.name,
                }))}
                defaultLabel={
                  !filters.selectedBrandId ? "Chọn hãng trước" : "Tất cả mẫu xe"
                }
              />
              <FilterSelect
                icon={DollarSign}
              ...
              <FilterSelect
                ...
                onChange={(e) =>
                  handleFilterChange("yearRange", e.target.value)
                }
                options={YEARS.map((y) => ({ value: y, label: String(y) }))}
                defaultLabel="Tất cả năm"
              />
            </div>
